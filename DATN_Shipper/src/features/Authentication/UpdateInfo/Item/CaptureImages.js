...
export default function CaptureImage({props, navigation, route}) {
  ...
  const capture = async () => {
    ...
    navigation.navigate('Update', {image: data, key: route?.params?.key});
  };
  const changeType = () => {
    type === 0
      ...
      <RNCamera
        ...
        <View style={styles.menuCamera}>
          <TouchableOpacity
            ...
            {flash === 0 ? (
              <Image style={styles.flashIcon} source={images.flashOff1} />
            ) : flash === 1 ? (
              <Image style={styles.flashIcon} source={images.flashOn} />
            ) : (
              <Image style={styles.flashIcon} source={images.flashAuto} />
            )}
          </TouchableOpacity>
          <TouchableOpacity style={styles.btnFlip} onPress={changeType}>
            <Image style={styles.flashIcon} source={images.filp} />
          </TouchableOpacity>
        </View>

        <TouchableOpacity style={styles.btnCapture} onPress={() => capture()}>
          <Image style={styles.captureIcon} source={images.capture} />
        </TouchableOpacity>
        <TouchableOpacity
